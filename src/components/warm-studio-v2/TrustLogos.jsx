...
const content = {
  Consulting: ['Hilo', 'NeuroQ', 'Life Seasons', 'Thirdbase Capital', 'E2Go', 'Protocol Health Club'],
  'B2B SaaS': ['Nothing Products', '4Knines', 'Rydura', 'Watchgod'],
  Logistics: ['Accolade', 'Stanford', 'Westrom Group', 'Ballyhoo Ventures', 'SB Sovrana', 'Kamiki'],
  Finance: ['Creative Launch', 'Rugtomize', 'Volado', 'Trackem', 'AuraReveal', 'Reply42', 'BG Imperial', 'Beaini Brands'],
};

const allClients = Object.values(content).flat();
      ...
      <div className="flex w-full flex-wrap justify-center gap-y-12 opacity-60 md:gap-y-24">
        {content[activeTab].map((company, index) => (
          <div
            ...
            className="w-1/2 animate-[trustLogoFade_0.5s_ease-out_forwards] text-center font-serif text-xl italic tracking-tight text-[var(--color-text-primary)] opacity-0 md:w-1/4 md:text-2xl"
...
function TrustLogosWall() {
  return (
    <>
      <ProofAndHeadline />

      <div className="grid w-full max-w-6xl grid-cols-2 gap-x-6 gap-y-12 opacity-60 sm:grid-cols-3 md:grid-cols-4 md:gap-y-16 lg:grid-cols-5">
        {allClients.map((company, index) => (
          <div
            key={`${company}-${index}`}
            className="animate-[trustLogoFade_0.5s_ease-out_forwards] text-center font-serif text-xl italic tracking-tight text-[var(--color-text-primary)] opacity-0 md:text-2xl"
...
/** @param {{ layout?: 'tabs' | 'wall' }} props */
export default function TrustLogos({ layout = 'tabs' }) {
  return (
    <section className="overflow-hidden border-b border-[var(--color-text-primary)]/5 bg-[var(--color-surface)] py-24">
      <div className="mx-auto flex max-w-7xl flex-col items-center px-6">
        {layout === 'wall' ? <TrustLogosWall /> : <TrustLogosTabs />}
